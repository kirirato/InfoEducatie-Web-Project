<?php
		...
		foreach($result as $item) {
			...
			echo '<div class="comment" name="'.$item['com_code'].'">'
				...
				if($chi_cnt == 0)
				{
					...
					echo '<span class="no_reply">No replies</span>';
						//.'<span class="replies">&emsp;Reply</span>';
				}
				else {
					echo '<span class="no_reply"> '.$chi_cnt.' replies</span>';
						//.'<span class="replies">&emsp;Reply</span>';
	...
	function add_comment($reply, $code, $file_id) {
		echo '<form action="reply.php" method="post" enctpye="" name="new_comment">'
			...
			.'<textarea class="text_cmt" name="username" placeholder="Name"></textarea><br />'
